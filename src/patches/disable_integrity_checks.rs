use windows::{
    ...
    Win32::System::Threading::{OpenThread, THREAD_ALL_ACCESS, TerminateThread},
};
...
fn check_thread(thread_id: u32) -> Result<bool, String> {
    unsafe {
        ...
        let thread_handle = match thread_handle {
            Ok(handle) => handle,
            Err(_) => return Err("failed to open thread handle".to_string()),
        };
        ...
        if thread_start_address == INTEGRITY_THREAD_START_ADDRESS {
            let result = TerminateThread(thread_handle, 0x0);
            if result.is_err() {
                return Err("failed to terminate thread".to_string());
            }
